"""
CFD++ GUI 调用关系分析器 v11 - 加 source 正则 fallback
"""
import os
import re
import json
...
def find_sources_regex(text):
    """正则式 fallback:在文件中找 'source FILENAME' 模式(处理 $variable 引用)"""
    results = []
    lines = text.split('\n')
    for i, line in enumerate(lines, 1):
        ...
        if stripped.startswith('#'):
            continue
        # 匹配 'source ${path}/file.tcl' 或 'source "$path/file.tcl"' 或 'source file.tcl'
...
def main():
    ...
    for idx, fn in enumerate(files):
        ...
        # 用 regex 补充 (处理 cfd++.tk 那种被 brace 吞掉的情况)
        regex_sources = find_sources_regex(text)
        # 合并去重
        seen = set(s[1] for s in sources)
        for line, target in regex_sources:
            if target not in seen:
                sources.append((line, target))
                seen.add(target)
        for p in procs:
    ...
    out_json = os.path.join(OUTPUT_DIR, 'analysis_v11.json')
    out = {}
    for fn, fd in fd_all.items():
        out[fn] = {
            'sources': fd['sources'],
