...
> 这是已知的设计简化(见 [`07-sweep-risks-roadmap`](../sweep/07-sweep-risks-roadmap.md) 风险登记)。
...
**CLI / API 层**会进一步包装(见 [`inp_tool/cli.py`](../../inp_tool/inp_tool/cli.py) 与 [`03-sweep-usage`](../sweep/03-sweep-usage.md) §3)。
...
> **关于 "11 个文件"**:任务描述里说 "11 个文件",实际统计为 13 个 `test_*.py` + 1 个 `conftest.py` = 14 个文件;`test_*.py` 中 4 个专门覆盖核心模块(`test_parser` / `test_writer` / `test_diff` / `test_api`),其他 9 个是 sweep 相关的。详见 [`06-sweep-testing`](../sweep/06-sweep-testing.md)。
...
- 整体架构(包结构图、数据流):[`01-architecture-overview`](01-architecture-overview.md)
- sweep 模块怎么用这些核心模块:[`02-sweep-architecture`](../sweep/02-sweep-architecture.md) §3
- CLI / API 速查(用户视角):[`../user-manual/reference/02-cli-api-reference`](../user-manual/reference/02-cli-api-reference.md)
- mcfd.inp 字段参考(数据视角):[`../user-manual/reference/01-mcfd-inp-field-reference`](../user-manual/reference/01-mcfd-inp-field-reference.md)
