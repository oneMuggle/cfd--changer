...
> 阅读路径:首次接触 → [01-介绍](01-introduction.md);想了解内部实现 → [../technical/](../technical/)。

---

## A

- **alpha(攻角)**:飞行器气动研究里的"迎角",单位是度(deg)。`inp_tool` 中作为 sweep 轴名,自动写进 `guiopts.aero_alpha` 并参与几何分解。详见 [04-扫描参数 §2](04-sweeping.md)。
...
- **ASCII**:`mcfd.inp` 是纯 ASCII 文本,无 BOM,无 UTF-16。Windows 下保存时务必用 ANSI / UTF-8 no BOM,否则 `inp-tool parse` 报编码错。详见 [10-FAQ](10-faq.md)。
- **AMR(Adaptive Mesh Refinement)**:自适应网格细化。`mcfd.inp` 的 `octree` 块管 AMR 参数;`inp_tool` 不自动改它,需走 [07-字段覆盖](07-overrides.md)。详见 [12-字段参考 §3.7](12-mcfd-inp-field-reference.md)。
- **auto_***:`guiopts` 块下另一组"自动推断"字段(`auto_pres/temp/u/v/w/tlvl/...`),求解器在某些场景会自动用它们覆盖 `aero_*`。`inp_tool` 默认不动 `auto_*`,需用户手动 [07-overrides](07-overrides.md)。
...
- **Cartesian product(笛卡尔积)**:sweep 的核心展开方式,N 个 sweep 轴的值表做"全配对",得到 `len(ax1) × len(ax2) × ...` 个 case。详见 [04-扫描参数 §4](04-sweeping.md)。
...
- **HTTPS_PROXY**:公司代理下 `pip install` 失败时设的环境变量(`set HTTPS_PROXY=http://your-proxy:8080`)。详见 [10-FAQ §安装](10-faq.md)。
- **HPC(High Performance Computing)**:超算集群环境。`inp_tool` 不直接调 SLURM/PBS/LSF,生成 `.inp` 后由用户/调度器跑。详见 [14-教程 5](14-software-tutorial.md)。
...
- **interactive(交互式 CLI)**:`inp-tool sweep -i`,走 prompt 序列让用户一步步填配置,所有字段有 default,回车接受。详见 [08-多入口 §3](08-multiple-uis.md)。

## J

- **JSON**:sweep 配置的默认格式(`sweep.json`),标准库自带,无需 extras。**适合 CI / 自动化 / 复现**。详见 [05-配置文件 §2](05-config-files.md)。
- **Jupyter Notebook**:交互式 Python 环境,`inp_tool` Python API 的常用舞台。详见 [09-例 5](09-examples.md)。
...
- **LHS / Sobol(Latin Hypercube / Sobol 序列)**:DOE 采样方法。`inp_tool` 自己不做,但 SALib 等库的结果可直接喂进 `sweeps` 的值列表。详见 [09-例 3](09-examples.md)。
...
- **quickstart(快速开始)**:专指 [03-quickstart](03-quickstart.md),5 分钟跑通第一个批量生成的教程。详见 [03-快速开始](03-quickstart.md)。
...
- **T_inf(来流温度) / p_inf(来流压强)**:`inp_tool` 的辅助 sweep 轴(不是 `guiopts` 原生字段),自动转成 `aero_temp / aero_pres` 和 `reftem / refpre`。详见 [04-扫描参数 §5](04-sweeping.md)。
