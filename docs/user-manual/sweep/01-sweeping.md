...
> 如果你的样例里这些字段位置不一样,见 [07-字段覆盖](./04-overrides.md) 手动指定。
...
> ⚠️ **方向假设:** 默认 `α` 影响垂直(W),`β` 影响侧滑(V)。如果你的 CFD++ 版本不是这个约定,看 [07-字段覆盖 §关闭 preset](./04-overrides.md) 手动给 `aero_u/v/w`。
...
下一步:[05-配置文件](./02-config-files.md) — JSON vs YAML vs CLI 快捷 怎么选。
