...
## 6. 桌面 GUI(`inp-tool-gui`)

适合:习惯图形界面、需要下拉选择变量、不会写 YAML 的同事。
...
启动后,顶部有 4 个标签页:**文件** / **检测** / **Sweep** / **对比**。点 **Sweep** 进入 sweep 配置页。

### 6.2 Sweep 标签页 v0.17 新特性

#### 顶部 3 个 label

模板路径 / 输出目录 / 命名模式三个输入框前面新增了文字标签,新手一眼能看明白。点旁边的「浏览...」按钮可以选文件/目录,失焦自动同步到 controller。
...
失焦时自动校验,**输入非法值会在状态栏报错**(如「`my.int` 第 2 项不是整数」),不会静默放过。
...
只有全部轴都有效时,「运行」/「运行(Dry)」按钮才亮起。

### 6.3 注意

- **模板路径必须是本机能读到的**(`inp-tool-gui` 直接读本地路径,不像 Web GUI 走服务器)
- **下拉列表的变量取决于当前加载的模板**——换一个模板,下拉内容会变;失效的旧轴会被自动标红
- **类型校验在失焦时触发**,输入中暂未完成的状态不会被误判

## 7. Shell 补全(可选,但用了回不去)
...
| 习惯图形界面、不会写 YAML | 桌面 GUI(`inp-tool-gui`) |
| 一次性"我就跑 6 个看看" | 交互式 -i |
...
下一步:[09-完整示例](./06-examples.md) — 端到端跑通 3 个真实场景。
