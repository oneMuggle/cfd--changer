...
**Status:** ✅ 已实现 v0.10.0 (commit 065cdb9)
...
- ❌ **跨 case 的方程间一致性自动校验**(如"切到 NONE 能量后自动清 vibtem"可选,本版只 warning,不开 auto-clean)— 见 §5
...
| `inp_tool/inp_tool/cli.py` | `sweep` 子命令加 `--strict-equations` flag | +30 / -0 |
...
class TurbulencePresetBase(ABC):
    ...
    # 当 apply() 在"切模型"后被调用,若 True,按 new_model 决定清掉哪些 turbi_*:
    ...
    #  - LAMINAR:清全部 turbi_*
    # 切模型信息通过 apply() 接受 model: TurbulenceModel 参数传入(改写签名)。
```
...
# 顶层三个开关(布尔,默认全 true)
equation_switches:
  turbulence: true   # false → 只做初始化,不动 eqnset_define
  energy: true
  gas: true
```

CLI 等价:`inp-tool sweep --no-switch-turbulence --no-switch-energy --no-switch-gas`。

### 4.6 Per-case I/L/U 覆盖

```yaml
turbulence:           # 顶层默认(v0.9.1 老契约)—— CaseSweep.turbulence 单值对象
  I: 0.01
  L: 0.01
  U_ref: 204
...
6. CLI 加 `--strict-equations` / `--no-switch-turbulence` / `--no-switch-energy` / `--no-switch-gas` flag
7. (可选) REPL 加 `set-turb` / `set-energy` / `set-gas` 命令
8. 写 `19-equation-sweep-extend.md`
9. 跑 `test_equation_rewrite.py` + 跑全量 `tests/` → 全 pass
10. 跑 `conda run -n cfdchanger ruff check . && mypy inp_tool/`
11. 提 PR(feature 分支),CI 绿后 merge
12. merge 后 bump `__version__` 0.9.1 → 0.10.0,打 tag
