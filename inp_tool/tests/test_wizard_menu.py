...
class TestRunMenu:
    ...
    def test_menu_picks_sweep(self, monkeypatch, tmp_path, capsys):
        ...
        responses = iter([
            ...
            "1",                  # 5. mode cartesian
            "{alpha: [0]}",       # 6. sweeps
            "n", "n", "n",        # 7. step_4b: 3 axis confirms (全 skip,Cartesian 走)
            "case_{alpha}",       # 8. naming
            "y",                  # 9. pbs confirm
            "",                   # 10. pbs naming (空=接受建议)
            "y",                  # 11. preview confirm
            "n",                  # 12. 不覆盖
        ])
        monkeypatch.setattr("builtins.input", lambda _: next(responses))
        run_menu()
...
class TestModuleExports:
    ...
    def test_run_sweep(self, monkeypatch, tmp_path):
        ...
        responses = iter([
            ...
            "{alpha: [0]}",       # 6. sweeps
            "n", "n", "n",        # 7. step_4b: 3 axis confirms (全 skip)
            "case_{alpha}",       # 8. naming
            "y",                  # 9. pbs confirm
            "",                   # 10. pbs naming
            "y",                  # 11. confirm continue
            "n",                  # 12. 不覆盖
        ])
        monkeypatch.setattr("builtins.input", lambda _: next(responses))
